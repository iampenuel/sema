...
export async function POST(request: Request) {
  ...
  if (!parsed.success) return NextResponse.json({ error: { code: "validation_failed", message: "Enter a story before asking Sema to organize it." } }, { status: 400 });
  ...
  if (safetyFlags.length) {
    ...
    return NextResponse.json({ error: { code: "safety_blocked", message: getSafeRedirect(safetyFlags) }, safetyFlags }, { status: 422 });
  }
  ...
  if (!result.ok || !result.data) {
    ...
    return NextResponse.json({ error: { code: result.error?.code ?? "provider_unavailable", message: SAFE_AI_ERROR_MESSAGE }, metadata: result.metadata }, { status: 503 });
  }
  ...
  if (!validated.ok) {
    if (provider.id !== "local") {
      ...
      if (local.ok && local.data) {
        ...
        return NextResponse.json({ draft: local.data, metadata: local.metadata, fallbackNotice: SAFE_AI_ERROR_MESSAGE });
      }
    }
    return NextResponse.json({ error: { code: validated.reason === "safety" ? "safety_blocked" : "validation_failed", message: SAFE_AI_ERROR_MESSAGE }, metadata: result.metadata }, { status: 422 });
  }
