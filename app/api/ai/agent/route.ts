...
export async function POST(request: Request) {
  ...
  if (!parsed.success) return NextResponse.json({ error: { code: "validation_failed", message: "I could not understand that request." } }, { status: 400 });
  ...
  if (safetyFlags.length) {
    ...
    return NextResponse.json({ reply: getSafeRedirect(safetyFlags), proposedActions: [], safetyFlags, blocked: true, metadata: { provider: "local", model: "local-safety", fallbackUsed: false } });
  }
  ...
  if (!result.ok || !parsedProposal?.success) {
    ...
    return NextResponse.json({ reply: "I can help open signal folders, list missing details, prepare the packet after permission, or read the safety note.", proposedActions: [], safetyFlags: [], metadata: result.metadata, fallbackNotice: SAFE_AI_ERROR_MESSAGE });
  }
  ...
  if (!safety.safe || !validated.ok) {
    ...
    return NextResponse.json({ reply: safety.redirect ?? "I can continue with the local session tools.", proposedActions: [], safetyFlags: [], metadata: { ...result.metadata, fallbackUsed: true }, fallbackNotice: SAFE_AI_ERROR_MESSAGE });
  }
  ...
  return NextResponse.json({ reply: parsedProposal.data.reply, proposedActions: validated.actions, safetyFlags: [], requiresReview: parsedProposal.data.requiresReview, metadata: result.metadata, fallbackNotice: result.metadata.fallbackUsed ? SAFE_AI_ERROR_MESSAGE : undefined });
}
