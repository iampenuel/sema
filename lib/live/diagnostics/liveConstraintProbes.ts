...
export async function runConstraintProbes(model: string, execute: ConstraintProbeExecutor, onProbe?: (result: ConstraintProbeResult) => void) {
  ...
  for (const feature of LIVE_CONSTRAINT_FEATURES) {
    ...
    try {
      result = { stage: "constraint_probe", feature, model, apiVersion: TOKEN_API_VERSION, fallbackUsed: false, ...(await execute(feature)) };
    } catch (error) {
      const failure = classifyDiagnosticFailure(error, "constraint_probe", feature);
      result = { stage: "constraint_probe", feature, model, apiVersion: TOKEN_API_VERSION, fallbackUsed: false, passed: false, tokenCreated: false, socketOpened: false, setupAccepted: false, ...failure };
    }
    results.push(result);
    onProbe?.(result);
...
export function createGeminiConstraintProbeExecutor(timeoutMs = 15_000): ConstraintProbeExecutor {
  ...
  return async (feature) => {
    ...
    const embeddedConfig = buildConstraintConfig(feature, config.voiceName);
    ...
    const outcome = await connector.connectConstrained({ name: token.name, expiresAt: expiresAt.toISOString() });
