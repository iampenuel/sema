...
export function AudioSignalCard({
  ...
  return (
    <section className="card rounded-lg p-5" aria-labelledby="audio-title">
      ...
      <p className="mt-1 text-sm text-muted">Sema does not classify audio or identify diseases from recordings. Audio is included only as a patient-generated observation.</p>

      <div className="mt-5 grid gap-5 lg:grid-cols-2">
        <div className="rounded-lg border border-ink/10 bg-white p-4">
          <h3 className="font-semibold text-ink">Browser recorder</h3>
          <div className="mt-4 flex flex-wrap gap-2">
            <button type="button" onClick={recorder.start} disabled={recorder.isRecording} className="inline-flex items-center gap-2 rounded-lg bg-ink px-4 py-2 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:bg-muted">
              <Mic className="h-4 w-4" aria-hidden="true" />
              Record
            </button>
            <button type="button" onClick={recorder.stop} disabled={!recorder.isRecording} className="inline-flex items-center gap-2 rounded-lg border border-ink/10 bg-white px-4 py-2 text-sm font-semibold text-ink disabled:cursor-not-allowed disabled:text-muted">
          ...
          {recorder.audioUrl ? (
            <form action={saveRecorded} className="mt-4 space-y-3">
              ...
              <label className="block">
                ...
                <input name="name" className="mt-1 w-full rounded-lg border border-ink/10 px-3 py-2 text-sm" defaultValue="Recorded audio signal" />
              </label>
              <TagGrid />
              <label className="block">
                <span className="text-sm font-semibold text-ink">Notes</span>
                <textarea name="notes" className="mt-1 min-h-20 w-full rounded-lg border border-ink/10 px-3 py-2 text-sm" />
              </label>
              <button type="submit" className="rounded-lg bg-sage px-4 py-2 text-sm font-semibold text-white">Save recording</button>
            </form>
          ) : (
        ...
        <form action={saveFallback} className="rounded-lg border border-ink/10 bg-white p-4">
          ...
          <label className="mt-4 block">
            ...
            <input name="fallbackName" className="mt-1 w-full rounded-lg border border-ink/10 px-3 py-2 text-sm" defaultValue="Manual audio note" />
          </label>
          <label className="mt-3 block">
            <span className="text-sm font-semibold text-ink">Duration in seconds</span>
            <input name="duration" type="number" min="1" className="mt-1 w-full rounded-lg border border-ink/10 px-3 py-2 text-sm" defaultValue={18} />
          </label>
          <div className="mt-3">
            <TagGrid />
          </div>
          <label className="mt-3 block">
            <span className="text-sm font-semibold text-ink">Notes</span>
            <textarea name="fallbackNotes" className="mt-1 min-h-20 w-full rounded-lg border border-ink/10 px-3 py-2 text-sm" placeholder="Describe the patient-generated audio observation without interpreting disease." />
          </label>
          <button type="submit" className="mt-3 rounded-lg bg-ink px-4 py-2 text-sm font-semibold text-white">Save audio note</button>
        </form>
      </div>
      ...
      <div className="mt-5">
        ...
        {session.audioSignals.length === 0 ? (
          <p className="mt-2 rounded-lg border border-dashed border-ink/20 bg-white/70 p-4 text-sm text-muted">No audio signal yet.</p>
        ) : (
          <ul className="mt-2 grid gap-2">
            {session.audioSignals.map((signal) => (
              <li key={signal.id} className="rounded-lg border border-ink/10 bg-white p-3">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <button type="button" onClick={() => onRemove(signal.id)} aria-label={`Remove ${signal.name}`} className="rounded-lg p-2 text-muted hover:bg-clay/10 hover:text-clay">
  ...
  return (
    <fieldset>
      ...
      <div className="mt-2 flex flex-wrap gap-2">
        {tags.map((tag) => (
          <label key={tag} className="inline-flex items-center gap-2 rounded-full border border-ink/10 bg-white px-3 py-1.5 text-sm text-ink">
...
function Waveform({ peaks }: { peaks: number[] }) {
  return (
    <div className="mt-3 flex h-12 items-center gap-1" aria-label="Decorative waveform preview">
      <Waves className="mr-2 h-4 w-4 text-sage" aria-hidden="true" />
      {peaks.map((peak, index) => (
        <span key={`${peak}-${index}`} className="w-2 rounded-full bg-sage/60" style={{ height: `${Math.max(10, peak)}%` }} />
      ))}
    </div>
  );
}
