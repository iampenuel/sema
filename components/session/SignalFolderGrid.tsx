...
export function SignalFolderGrid({
  session,
  activeFolder,
  onOpen,
  sectionRef,
  ...
  onOpen: (folder: SignalFolderId) => void;
  ...
  return (
    <section ref={sectionRef} aria-labelledby="signal-folders-title" className="scroll-mt-24" data-workspace-target="folder_grid">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-xs font-bold text-sema-blue">SIGNAL CASE FILE</p>
          <h2 id="signal-folders-title" ref={headingRef} tabIndex={-1} className="mt-1 font-editorial text-3xl font-semibold text-ink outline-none focus-visible:ring-2 focus-visible:ring-sema-blue">Build your evidence folders.</h2>
        </div>
      ...
      <div className="mt-5 grid auto-rows-fr gap-4 md:grid-cols-2">
        {folderDefinitions.map((folder) => {
          ...
          const status = needsReview ? "Needs review" : isSaved ? "Saved" : folderStatus === "not_applicable" ? "Not applicable" : folderStatus === "planned_later" ? "Planned later" : isRecommended ? "Next recommended" : folderStatus === "in_progress" ? "In progress" : "Empty";
          ...
          return (
            <article key={folder.id} className="relative h-full pt-4">
              ...
              <button type="button" data-workspace-folder={folder.id} onClick={() => onOpen(folder.id)} aria-pressed={isActive} className={`group flex h-full min-h-[184px] w-full flex-col rounded-lg rounded-tl-none border bg-gradient-to-br from-[#e6f3fa] via-[#eef7fb] to-[#f8fbfd] p-5 text-left shadow-card transition duration-200 hover:-translate-y-0.5 hover:border-sema-blue hover:shadow-soft ${isActive ? "border-sema-blue ring-2 ring-sema-blue/10" : isRecommended ? "border-sema-blue/60" : "border-[#b9d2e3]"}`}>
